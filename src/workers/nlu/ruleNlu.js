...
export function isDeclineAlcohol(hay, text) {
  ...
  return (
    ...
    /금주|단주|노알콜|논알콜만|알콜\s*없이|술\s*없이|알콜\s*안받|술마실\s*기분\s*아니|간\s*쉬|속\s*안좋아서\s*술|술\s*생각\s*(?:1도\s*)?없/.test(h) ||
    /술(?:은|이|도|을)?\s*(?:안\s*마|안\s*땡|안\s*먹|생각.*없|못\s*마|안\s*끌|그만|패스|빼|싫|안당겨|됐)/.test(t) ||
    /금주|단주|노알콜|논알콜만/.test(t)
  );
}
